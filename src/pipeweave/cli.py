...
def create_pipeline_from_config(config: Dict[str, Any]) -> Pipeline:
    ...
    for step_config in config['steps']:
        ...
        if not all(field in step_config for field in required_step_fields):
            raise ValueError(f"Step configuration missing required fields: {required_step_fields}")
...
@click.option('--input-data', '-i', type=click.Path(exists=True), help='Path to input data file (JSON/YAML)')
...
def run(config_path: str, db_path: Optional[str], input_data: Optional[str], 
        save: bool, output: Optional[str]) -> None:
...
def delete(db_path: str, pipeline_name: str) -> None:
    ...
    """
    try:
        if click.confirm(f"Are you sure you want to delete pipeline '{pipeline_name}'?"):
            storage = SQLiteStorage(db_path)
    ...
    try:
        ...
        if pipeline.stages:
            ...
            for stage_name, stage in pipeline.stages.items():
                ...
                click.echo(f"    Dependencies: {', '.join(stage.dependencies) if stage.dependencies else 'None'}")
                click.echo("    Steps:")
